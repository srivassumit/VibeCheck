...
interface IAnalysisRun extends Document {
  title: string;
  description: string;
  prLink: string;
  requirements: string;
...
const AnalysisRun = mongoose.model<IAnalysisRun>('AnalysisRun', AnalysisRunSchema, 'PRChecks');
...
app.post('/api/history', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`💾 Saved session to PRChecks: ${(saved._id as mongoose.Types.ObjectId)}`);
